...
Country_Txn_Data_2_UDV <- sqldf(paste0("Select * from Country_Txn_Data_1 where 
...
##################### CR_HK_CNP_STIP_LOC_15MIN_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, CR_HK_CNP_STIP_LOC_15MIN_CNT:=seq_len(.N), by=list(MERCHANTBIN, cumsum(dt1$group_10 >= 1))]

##################### CR_HK_CNP_STIP_LOC_15MIN_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$CR_HK_CNP_STIP_LOC_15MIN_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$CR_HK_CNP_STIP_LOC_15MIN_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$CR_HK_CNP_STIP_LOC_15MIN_CNT <- as.numeric(as.character(Country_Txn_Data_2$CR_HK_CNP_STIP_LOC_15MIN_CNT))
Country_Txn_Data_2$CR_HK_CNP_STIP_LOC_15MIN_AMT <- as.numeric(as.character(Country_Txn_Data_2$CR_HK_CNP_STIP_LOC_15MIN_AMT))
