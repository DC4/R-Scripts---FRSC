...
# Common = COMPROMISE_NCCC_CR

COMPROMISE_NCCC_CR <- read_excel("//inhadfil101.in.standardchartered.com/wkgrps5/RAC_IN_MIS/Sumeet/R_Simulation/Hotlist/COMPROMISE_NCCC.xls")

COMPROMISE_NCCC_CR <- data.table(COMPROMISE_NCCC_CR)

Name_list_TW_CR <- sqldf("Select Name from COMPROMISE_NCCC_CR")

Country_Txn_Data_1_sub <- sqldf("Select * from Country_Txn_Data_Hot_CR where Trim(ACCT_NBR) in Name_list_TW_CR ")
...
# Common = COMPROMISE_NCCC_DR

COMPROMISE_NCCC_DR <- read_excel("//inhadfil101.in.standardchartered.com/wkgrps5/RAC_IN_MIS/Sumeet/R_Simulation/Hotlist/COMPROMISE_NCCC.xls")

COMPROMISE_NCCC_DR <- data.table(COMPROMISE_NCCC_DR)

Name_list_TW_DB <- sqldf("Select Name from COMPROMISE_NCCC_DR")

Country_Txn_Data_1_sub_1 <- sqldf("Select * from Country_Txn_Data_Hot_DB where Trim(ACCT_NBR) in Name_list_TW_DB ")
