...
dt[, SRVC_UDV_CNP3trns_TIME := floor_date(as.POSIXct(date_2) + time_2, "5 minutes")]

# count and sum by 10 Minutes
dt[,`:=`(SRVC_UDV_CNP3trns_CNT = as.numeric(seq_len(.N)), SRVC_UDV_CNP3trns_AMT = as.numeric(cumsum(USD), digits = 2)) , by = .(ACCT_NBR, SRVC_UDV_CNP3trns_TIME)]
...
dt1$SRVC_UDV_CNP3trns_TIME = as.POSIXct("01.01.1960 00:00:00", format = "%d.%m.%Y %H:%M:%S", tz = "GMT");
dt1$SRVC_UDV_CNP3trns_CNT = ""
dt1$SRVC_UDV_CNP3trns_AMT = ""
...
Country_Txn_Data$SRVC_UDV_CNP3trns_CNT <- as.numeric(as.character(Country_Txn_Data$SRVC_UDV_CNP3trns_CNT))
Country_Txn_Data$SRVC_UDV_CNP3trns_AMT <- as.numeric(as.character(Country_Txn_Data$SRVC_UDV_CNP3trns_AMT))
...
UDV_Calculation_Table_Ref_1 <- Country_Txn_Data %>%
  select(ACCT_NBR, AUTH_DTTM_2, TRN_AUTH_POST, AUTH_DECISION_XCD, TRN_POS_ENT_CD, TRN_TYP, USD, date_2, time_2, SRVC_UDV_CNP3trns_TIME, SRVC_UDV_CNP3trns_CNT, SRVC_UDV_CNP3trns_AMT)
