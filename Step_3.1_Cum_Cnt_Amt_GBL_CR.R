...
CRD_CLNT_ID IN ('SC_C400NG_CR', 'SC_CCMSIN_CR', 'SC_C400NP_CR', 'SC_C400ZM_CR') and
TRN_AUTH_POST = 'A' and
...
##################### CR_GBL_SRVC_UDV_CNP_5MIN_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, CR_GBL_SRVC_UDV_CNP_5MIN_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### CR_GBL_SRVC_UDV_CNP_5MIN_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$CR_GBL_SRVC_UDV_CNP_5MIN_AMT <- unlist(tapply(dt1$TRN_AMT, cumsum(c(0, diff(dt1$CR_GBL_SRVC_UDV_CNP_5MIN_CNT) < 1)), cumsum))
...
Country_Txn_Data_1$CR_GBL_SRVC_UDV_CNP_5MIN_CNT <- as.numeric(as.character(Country_Txn_Data_1$CR_GBL_SRVC_UDV_CNP_5MIN_CNT))
Country_Txn_Data_1$CR_GBL_SRVC_UDV_CNP_5MIN_AMT <- as.numeric(as.character(Country_Txn_Data_1$CR_GBL_SRVC_UDV_CNP_5MIN_AMT))
