...
(USER_DATA_4_STRG_New NOT IN ('', '00000000000') AND length(USER_DATA_4_STRG) > 1) and
USD > 0"))
# MER_CNTY_CD != 344 and
...
##################### CR_HK_SRVC_Set_TOKEN_TRANSACTION_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, CR_HK_SRVC_Set_TOKEN_TRANSACTION_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### CR_HK_SRVC_Set_TOKEN_TRANSACTION_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$CR_HK_SRVC_Set_TOKEN_TRANSACTION_AMT <- unlist(tapply(dt1$TRN_AMT, cumsum(c(0, diff(dt1$CR_HK_SRVC_Set_TOKEN_TRANSACTION_CNT) < 1)), cumsum))
...
Country_Txn_Data_1$CR_HK_SRVC_Set_TOKEN_TRANSACTION_CNT <- as.numeric(as.character(Country_Txn_Data_1$CR_HK_SRVC_Set_TOKEN_TRANSACTION_CNT))
Country_Txn_Data_1$CR_HK_SRVC_Set_TOKEN_TRANSACTION_AMT <- as.numeric(as.character(Country_Txn_Data_1$CR_HK_SRVC_Set_TOKEN_TRANSACTION_AMT))
