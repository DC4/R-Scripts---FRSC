...
CRD_CLNT_ID not in ('SC_TANDEMTW_DB', 'SC_CCMSHK_CR', 'SC_PMTHK_CR', 'SC_C400BD_CR') AND 
TRN_AUTH_POST = 'A' and
...
##################### GBL_CR_CNP_DAILY_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, GBL_CR_CNP_DAILY_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### GBL_CR_CNP_DAILY_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$GBL_CR_CNP_DAILY_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$GBL_CR_CNP_DAILY_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$GBL_CR_CNP_DAILY_CNT <- as.numeric(as.character(Country_Txn_Data_2$GBL_CR_CNP_DAILY_CNT))
Country_Txn_Data_2$GBL_CR_CNP_DAILY_AMT <- as.numeric(as.character(Country_Txn_Data_2$GBL_CR_CNP_DAILY_AMT))
