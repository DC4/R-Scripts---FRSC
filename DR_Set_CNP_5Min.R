...
Country_Txn_Data_1_UDV <- sqldf(paste0("Select * from Country_Txn_Data_1
 where TRN_AUTH_POST = 'A' and
AUTH_DECISION_XCD = 'A' and (TRN_POS_ENT_CD in ('E', 'K', 'G') or (TRN_POS_ENT_CD = '' and USR_IND_4_New_2 = 'Y')) and TRN_TYP in ('C', 'M', 'P')"))
# MER_CNTY_CD != 344 and
...
##################### DR_SRVC_UDV_CNP_5MIN_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DR_SRVC_UDV_CNP_5MIN_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### DR_SRVC_UDV_CNP_5MIN_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$DR_SRVC_UDV_CNP_5MIN_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$DR_SRVC_UDV_CNP_5MIN_CNT) < 1)), cumsum))
...
Country_Txn_Data_1$DR_SRVC_UDV_CNP_5MIN_CNT <- as.numeric(as.character(Country_Txn_Data_1$DR_SRVC_UDV_CNP_5MIN_CNT))
Country_Txn_Data_1$DR_SRVC_UDV_CNP_5MIN_AMT <- as.numeric(as.character(Country_Txn_Data_1$DR_SRVC_UDV_CNP_5MIN_AMT))
