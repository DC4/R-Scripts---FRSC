...
TRN_POS_ENT_CD IN ('E', 'K', 'G') or (TRN_POS_ENT_CD = '' and USR_IND_4_NEW_2 = 'Y') and 
...
##################### DB_GBL_SRVC_UDV_CNP_DAILY_CNT_1 ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DB_GBL_SRVC_UDV_CNP_DAILY_CNT_1:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### DB_GBL_SRVC_UDV_CNP_DAILY_AMT_1 ##########################

# Getting the CUM_AMOUNT count variable

dt1$DB_GBL_SRVC_UDV_CNP_DAILY_AMT_1 <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$DB_GBL_SRVC_UDV_CNP_DAILY_CNT_1) < 1)), cumsum))
...
Country_Txn_Data_2$DB_GBL_SRVC_UDV_CNP_DAILY_CNT_1 <- as.numeric(as.character(Country_Txn_Data_2$DB_GBL_SRVC_UDV_CNP_DAILY_CNT_1))
Country_Txn_Data_2$DB_GBL_SRVC_UDV_CNP_DAILY_AMT_1 <- as.numeric(as.character(Country_Txn_Data_2$DB_GBL_SRVC_UDV_CNP_DAILY_AMT_1))
