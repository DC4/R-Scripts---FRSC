...
CRD_CLNT_ID NOT IN ('SC_TANDEMTW_DB', 'SC_HOGANHK_DB', 'SC_SPARROWBD_DB', 'SC_EURONETID_DB') and
TRN_AUTH_POST = 'A' and
...
##################### DR_GBL_CNP_DAILY_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DR_GBL_CNP_DAILY_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### DR_GBL_CNP_DAILY_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$DR_GBL_CNP_DAILY_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$DR_GBL_CNP_DAILY_CNT) < 1)), cumsum))
...
Country_Txn_Data_1$DR_GBL_CNP_DAILY_CNT <- as.numeric(as.character(Country_Txn_Data_1$DR_GBL_CNP_DAILY_CNT))
Country_Txn_Data_1$DR_GBL_CNP_DAILY_AMT <- as.numeric(as.character(Country_Txn_Data_1$DR_GBL_CNP_DAILY_AMT))
