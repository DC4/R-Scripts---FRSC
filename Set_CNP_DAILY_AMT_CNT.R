...
Set_CNP_DAILY_TIME = as.POSIXct("01.01.1960 00:00:00", format = "%d.%m.%Y %H:%M:%S", tz = "GMT");
# Initializing date format
# Country_Txn_Data$Set_CNP_DAILY_TIME = Set_CNP_DAILY_TIME
...
dt[, Set_CNP_DAILY_TIME := floor_date(as.POSIXct(date_2) + time_2, "24 hours")]

# count and sum by 10 Minutes
dt[,`:=`(Set_CNP_DAILY_CNT = as.numeric(seq_len(.N)), Set_CNP_DAILY_AMT = as.numeric(cumsum(USD), digits = 2)) , by = .(ACCT_NBR, Set_CNP_DAILY_TIME)]
...
dt1$Set_CNP_DAILY_TIME = as.POSIXct("01.01.1960 00:00:00", format = "%d.%m.%Y %H:%M:%S", tz = "GMT");
dt1$Set_CNP_DAILY_CNT = ""
dt1$Set_CNP_DAILY_AMT = ""
...
Country_Txn_Data$Set_CNP_DAILY_CNT <- as.numeric(as.character(Country_Txn_Data$Set_CNP_DAILY_CNT))
Country_Txn_Data$Set_CNP_DAILY_AMT <- as.numeric(as.character(Country_Txn_Data$Set_CNP_DAILY_AMT))
...
UDV_Calculation_Table_Ref_1 <- Country_Txn_Data %>%
  select(ACCT_NBR, AUTH_DTTM_2, TRN_AUTH_POST, AUTH_DECISION_XCD, TRN_POS_ENT_CD, TRN_TYP, USD, date_2, time_2, Set_CNP_DAILY_TIME, Set_CNP_DAILY_CNT, Set_CNP_DAILY_AMT)
