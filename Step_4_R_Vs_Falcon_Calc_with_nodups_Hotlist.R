...
# Converting tibble to data frame to use for subsetting
Rule_Name_Temp_1 = as.data.frame(mydata[i,4])
Rule_Name_Temp_2 = gsub("'", '', Rule_Name_Temp_1)

# Check if the rule data set Country_Txn_Data_1 has the column Hotlist_Rule_Name and then segregate data
if("Hotlist_Rule_Name" %in% colnames(Country_Txn_Data_1))
{
# Checks in Txn data if there is any data specific to rule based on hotlist alone
HoT_NotBlank <- subset(Country_Txn_Data_1, Hotlist_Rule_Name == Rule_Name_Temp_2)

print("Rule Name: ")
print(mydata[i,4])
print("Hotist data if available: ")
dim(HoT_NotBlank)

# Hotlist code
Txn_Data_Fromat <- paste0("Select * from HoT_NotBlank WHERE ", mydata[i,2], " and " , 
" UPPER(cnty) in ( ", mydata$Tenant[i] ," ) ", " and LOWER(Dbname) like '",proj,"%'",
" and UPPER(Hotlist_Rule_Name) = ", mydata[i,4]," and trn_dt1 >= '" , type_dt, " '")
} else {
