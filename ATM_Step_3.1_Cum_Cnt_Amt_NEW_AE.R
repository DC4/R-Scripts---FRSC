...
Country_Txn_Data_1 <- sqldf(paste0('Select * from Country_Txn_Data_1 where CRD_CLNT_ID = "SC_C400AE_CR"'))
...
TRN_AUTH_POST = "A" AND MER_ID <> "" AND Txn_No >= "1" AND TRN_TYP IN ("C", "M", "P") AND (AUTH_DECISION_XCD = "A" OR DECI_CD_ORIG = "A") AND TRN_AMT > 0'))
...
# Get the First Apprv date for each txn
xxx_1 <- dt3 %>% 
  group_by(ATMONUS) %>% 
  summarize(Min_Date = min(AUTH_DTTM_2))

# Converting to Data frame
yyy_1 <- data.frame(xxx_1)
...
# dt1 %>%
  # select(ATMONUS, TRN_AUTH_POST, AUTH_DECISION_XCD, TRN_TYP, TRN_POS_ENT_CD, USR_IND_4_NEW, FRD_SCOR,  MER_NM, AUTH_DTTM_2, datetimediff_mins, group_10, Txn_No, ATMONUS_FIRST_TXN_DATE, ATMONUS_FIRST_APPROVAL_DATE)

  
# dt1 %>%
  # select(ATMONUS, AUTH_DTTM_2, datetimediff, datetimediff_mins, group_10, Txn_No, ATMONUS_FIRST_TXN_DATE, ATMONUS_FIRST_APPROVAL_DATE, TRN_AMT)
...
# dt1 %>%
  # select(ATMONUS, CUM_COUNT_ATMONUS_DAILY, TRN_AMT, CUM_AMOUNT_ATMONUS_DAILY)

##############################  Step : 2  #########################

# Getting the APPR variables

dt_Apr <- sqldf(paste0('Select * from dt1 where 
TRN_AUTH_POST = "A" AND MER_ID <> "" AND TRN_TYP IN ("C", "M", "P") AND (AUTH_DECISION_XCD = "A" OR DECI_CD_ORIG = "A") AND TRN_AMT > 0'))

# Recods not matching the condition
dt_Apr_2 <- anti_join(dt1, dt_Apr, by = "FI_TRANSACTION_ID")

# Sorting before performing cumulative count and cumulative amount
dt_Apr <- dt_Apr[order(dt_Apr$ATMONUS,dt_Apr$AUTH_DTTM_2), ]

# Getting the CUM_COUNT_APPR_ATMONUS_DAILY count variable
# setDT(dt_Apr)[, CUM_COUNT_APPR_ATMONUS_DAILY:=as.numeric(seq(1, length(dt_Apr$ATMONUS), by = 1)), by=list(ATMONUS, cumsum(dt_Apr$group_10 >= 1))]

# Getting the CUM_COUNT_APPR_ATMONUS_DAILY count variable
dt_Apr$CUM_COUNT_APPR_ATMONUS_DAILY <- unlist(tapply(as.numeric(seq(0, length(dt_Apr$ATMONUS), by = 1)), cumsum(c(0, diff(dt_Apr$AUTH_DTTM_2) < 1)), cumsum, default = 0))

# Getting the CUM_AMOUNT_APPR_ATMONUS_DAILY count variable
dt_Apr$CUM_AMOUNT_APPR_ATMONUS_DAILY <- unlist(tapply(dt_Apr$TRN_AMT, cumsum(c(0, diff(dt_Apr$CUM_COUNT_APPR_ATMONUS_DAILY) < 1)), cumsum, default = 0))

# Converting to Data frame
dt_Apr_1 <- data.frame(dt_Apr)

dt1 <- rbind(dt_Apr_1, dt_Apr_2, fill=TRUE)

# Sorting General
dt1 <- dt1[order(dt1$ATMONUS,dt1$AUTH_DTTM_2), ]

dt1 %>%
  select(ATMONUS, AUTH_DTTM_2, TRN_AMT, group_10, Txn_No, CUM_COUNT_APPR_ATMONUS_DAILY, CUM_AMOUNT_APPR_ATMONUS_DAILY)
