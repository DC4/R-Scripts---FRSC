...
dt$datetimediff = round(as.integer(dt$datetimediff), digits = 2)
...
##################### DR_GBL_Set_CNP_Hourly_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DR_GBL_Set_CNP_Hourly_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### DR_GBL_Set_CNP_Hourly_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$DR_GBL_Set_CNP_Hourly_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$DR_GBL_Set_CNP_Hourly_CNT) < 1)), cumsum))
...
Country_Txn_Data_1$DR_GBL_Set_CNP_Hourly_CNT <- as.numeric(as.character(Country_Txn_Data_1$DR_GBL_Set_CNP_Hourly_CNT))
Country_Txn_Data_1$DR_GBL_Set_CNP_Hourly_AMT <- as.numeric(as.character(Country_Txn_Data_1$DR_GBL_Set_CNP_Hourly_AMT))
