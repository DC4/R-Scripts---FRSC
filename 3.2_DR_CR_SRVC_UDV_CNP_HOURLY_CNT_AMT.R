...
##################### DR_CR_UDV_CNP_HRLY_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DR_CR_UDV_CNP_HRLY_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### DR_CR_UDV_CNP_HRLY_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$DR_CR_UDV_CNP_HRLY_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$DR_CR_UDV_CNP_HRLY_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$DR_CR_UDV_CNP_HRLY_CNT <- as.numeric(as.character(Country_Txn_Data_2$DR_CR_UDV_CNP_HRLY_CNT))
Country_Txn_Data_2$DR_CR_UDV_CNP_HRLY_AMT <- as.numeric(as.character(Country_Txn_Data_2$DR_CR_UDV_CNP_HRLY_AMT))
