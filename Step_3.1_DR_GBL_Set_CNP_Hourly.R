...
Country_Txn_Data_1 <- data.table(Country_Txn_Data_DB)
# Getting only CR / DB data for udv
Country_Txn_Data_1 <- Country_Txn_Data_1[which(str_sub(Country_Txn_Data_1$CRD_CLNT_ID,-3) == '_DB'), ]
...
##################### DR_GBL_SRVC_UDV_CNP_HOURLY_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DR_GBL_SRVC_UDV_CNP_HOURLY_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### DR_GBL_SRVC_UDV_CNP_HOURLY_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$DR_GBL_SRVC_UDV_CNP_HOURLY_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$DR_GBL_SRVC_UDV_CNP_HOURLY_CNT) < 1)), cumsum))
...
Country_Txn_Data_1$DR_GBL_SRVC_UDV_CNP_HOURLY_CNT <- as.numeric(as.character(Country_Txn_Data_1$DR_GBL_SRVC_UDV_CNP_HOURLY_CNT))
Country_Txn_Data_1$DR_GBL_SRVC_UDV_CNP_HOURLY_AMT <- as.numeric(as.character(Country_Txn_Data_1$DR_GBL_SRVC_UDV_CNP_HOURLY_AMT))
