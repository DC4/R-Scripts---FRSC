...
# setDT(dt1)[, ATMONUS_FIRST_TXN_DATE:=seq_len(.N), by=list(ATMONUS, cumsum(dt1$group_10 >= 1))]
# setDT(dt1)[, ATMONUS_FIRST_APPROVAL_DATE:=seq_len(.N), by=list(ATMONUS, cumsum(dt1$group_10 >= 1))]
...
df11 <- subset(dt1 , TRN_AUTH_POST == 'A' & MER_ID != '' & ATMONUS_FIRST_TXN_DATE == 0)
...
df11$ATMONUS_FIRST_APPROVAL_DATE <- as.POSIXct(df11$ATMONUS_FIRST_APPROVAL_DATE, origin = '2011-07-15 13:00:00')
...
df55 <- subset(dt1 , TRN_TYP %in% c('C', 'M', 'P') & (AUTH_DECISION_XCD == 'A' | DECI_CD_ORIG == 'A') & TRN_AMT > 0)
# removing subset data frame
dt1 <- anti_join(dt1,df55)
df55$CUM_COUNT_APPR_ATMONUS_DAILY = 1 ;
df55$CUM_AMOUNT_APPR_ATMONUS_DAILY = dt1$TRN_AMT;
# Appending the rows after logic is applied
dt1 <- rbind(dt1, df55)
