...
Country_Txn_Data_2 <- data.table(Country_Txn_Data_CR)
...
# dt[ , datetimediff_INV_EXP_CP := AUTH_DTTM_2 - lag_date,by=.(MERCHANTBIN)]
dt[ , datetimediff_INV_EXP_CP := difftime(AUTH_DTTM_2, lag_date, units = "mins"),by=.(MERCHANTBIN)]
dt[,datetimediff_INV_EXP_CP_mins := as.numeric(datetimediff_INV_EXP_CP)]
dt$datetimediff_INV_EXP_CP_mins[is.na(dt$datetimediff_INV_EXP_CP_mins)] <- 0
# Rounding datetimediff_INV_EXP_CP
# To eliminate the error "Error in asfn(rs[[i]]) : need explicit units for numeric conversion" - we remove the attribute
dt$datetimediff_INV_EXP_CP = round(as.numeric(dt$datetimediff_INV_EXP_CP), digits = 2)
...
dt1 <- as.data.table(dt %>% group_by() %>%
  mutate(
    group_10 = sapply(dt$datetimediff_INV_EXP_CP_mins, cumsum_with_reset_group)
  )  %>% 
  ungroup())


##################### INV_EXP_CP_INT_15MIN_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, INV_EXP_CP_INT_15MIN_CNT:=seq_len(.N), by=list(MERCHANTBIN, cumsum(dt1$group_10 >= 1))]

##################### INV_EXP_CP_INT_15MIN_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$INV_EXP_CP_INT_15MIN_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$INV_EXP_CP_INT_15MIN_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$INV_EXP_CP_INT_15MIN_CNT <- as.numeric(as.character(Country_Txn_Data_2$INV_EXP_CP_INT_15MIN_CNT))
Country_Txn_Data_2$INV_EXP_CP_INT_15MIN_AMT <- as.numeric(as.character(Country_Txn_Data_2$INV_EXP_CP_INT_15MIN_AMT))
