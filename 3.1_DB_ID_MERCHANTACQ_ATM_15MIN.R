...
##################### DR_ID_MERACQ_CP_15min_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DR_ID_MERACQ_CP_15min_CNT:=seq_len(.N), by=list(MERCHANTBIN, cumsum(dt1$group_10 >= 1))]

##################### DR_ID_MERACQ_CP_15min_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$DR_ID_MERACQ_CP_15min_AMT <- unlist(tapply(dt1$TRN_AMT, cumsum(c(0, diff(dt1$DR_ID_MERACQ_CP_15min_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$DR_ID_MERACQ_CP_15min_CNT <- as.numeric(as.character(Country_Txn_Data_2$DR_ID_MERACQ_CP_15min_CNT))
Country_Txn_Data_2$DR_ID_MERACQ_CP_15min_AMT <- as.numeric(as.character(Country_Txn_Data_2$DR_ID_MERACQ_CP_15min_AMT))
...
# Before Joining Country_Txn_Data_2 with Country_Txn_Data_1
...
# After Joining Country_Txn_Data_2 with Country_Txn_Data_1
dim(Country_Txn_Data_1)
head(Country_Txn_Data_1)