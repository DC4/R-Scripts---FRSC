...
# Atmonus Variable creation
Country_Txn_Data$ATMONUS <- paste0(trimws(Country_Txn_Data$MER_ID), trimws(Country_Txn_Data$ACCT_NBR))
...
Country_Txn_Data <- subset(Country_Txn_Data, !duplicated(subset(Country_Txn_Data, select=c(FI_TRANSACTION_ID))))
