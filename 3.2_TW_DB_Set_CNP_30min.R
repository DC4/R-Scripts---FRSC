...
setDT(dt1)[, TW_DR_CNP_30MIN_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### TW_DR_CNP_30MIN_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$TW_DR_CNP_30MIN_AMT <- unlist(tapply(dt1$TRN_AMT, cumsum(c(0, diff(dt1$TW_DR_CNP_30MIN_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$TW_DR_CNP_30MIN_CNT <- as.numeric(as.character(Country_Txn_Data_2$TW_DR_CNP_30MIN_CNT))
Country_Txn_Data_2$TW_DR_CNP_30MIN_AMT <- as.numeric(as.character(Country_Txn_Data_2$TW_DR_CNP_30MIN_AMT))
