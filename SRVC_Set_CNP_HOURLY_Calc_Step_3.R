...
TIME_RESET_POS91_DAILY = as.POSIXct("01.01.1960 00:00:00", format = "%d.%m.%Y %H:%M:%S", tz = "GMT");
# Initializing date format
# Country_Txn_Data$TIME_RESET_POS91_DAILY = TIME_RESET_POS91_DAILY
...
dt[, SRVC_Set_CNP_HOURLY_TIME := floor_date(as.POSIXct(date_2) + time_2, "60 minutes")]

# count and sum by 10 Minutes
dt[,`:=`(SRVC_Set_CNP_HOURLY_CNT = as.numeric(seq_len(.N)), SRVC_Set_CNP_HOURLY_AMT = as.numeric(cumsum(TRN_AMT), digits = 2)) , by = .(ACCT_NBR, SRVC_Set_CNP_HOURLY_TIME)]
...
dt1$SRVC_Set_CNP_HOURLY_TIME = as.POSIXct("01.01.1960 00:00:00", format = "%d.%m.%Y %H:%M:%S", tz = "GMT");
dt1$SRVC_Set_CNP_HOURLY_CNT = ""
dt1$SRVC_Set_CNP_HOURLY_AMT = ""
...
Country_Txn_Data$SRVC_Set_CNP_HOURLY_CNT <- as.numeric(as.character(Country_Txn_Data$SRVC_Set_CNP_HOURLY_CNT))
Country_Txn_Data$SRVC_Set_CNP_HOURLY_AMT <- as.numeric(as.character(Country_Txn_Data$SRVC_Set_CNP_HOURLY_AMT))
...
UDV_Calculation_Table_Ref_1 <- Country_Txn_Data %>%
  select(ACCT_NBR, AUTH_DTTM_2, TRN_AUTH_POST, AUTH_DECISION_XCD, TRN_POS_ENT_CD, TRN_TYP, USD, date_2, time_2, SRVC_Set_CNP_HOURLY_TIME, SRVC_Set_CNP_HOURLY_CNT, SRVC_Set_CNP_HOURLY_AMT)
...
Country_Txn_Data$USR_IND_4_New <- str_sub(Country_Txn_Data$USR_IND_4,1,2)
Country_Txn_Data$USR_IND_4_New_1 <- str_sub(Country_Txn_Data$USR_IND_4,3,4)
Country_Txn_Data$ACCT_NBR_New <- str_sub(Country_Txn_Data$ACCT_NBR,1,1)
Country_Txn_Data$USER_DATA_4_STRG_New <- str_sub(Country_Txn_Data$USER_DATA_4_STRG, 1, 11)
Country_Txn_Data$USR_DAT_2_New <- str_sub(Country_Txn_Data$USR_DAT_2, 9, 1)
Country_Txn_Data$USR_DAT_1_New <- str_sub(Country_Txn_Data$USR_DAT_1, 9, 2)
# Transaction data
