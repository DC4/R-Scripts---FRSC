...
##################### CR_GBL_SRVC_UDV_CNP_HOURLY_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, CR_GBL_SRVC_UDV_CNP_HOURLY_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### CR_GBL_SRVC_UDV_CNP_HOURLY_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$CR_GBL_SRVC_UDV_CNP_HOURLY_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$CR_GBL_SRVC_UDV_CNP_HOURLY_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$CR_GBL_SRVC_UDV_CNP_HOURLY_CNT <- as.numeric(as.character(Country_Txn_Data_2$CR_GBL_SRVC_UDV_CNP_HOURLY_CNT))
Country_Txn_Data_2$CR_GBL_SRVC_UDV_CNP_HOURLY_AMT <- as.numeric(as.character(Country_Txn_Data_2$CR_GBL_SRVC_UDV_CNP_HOURLY_AMT))
