...
CRD_CLNT_ID IN ('SC_C400BW_CR', 'SC_C400NG_CR', 'SC_C400KE_CR', 'SC_C400JE_CR', 'SC_C400NP_CR', 'SC_C400ZM_CR', 'SC_C400JO_CR', 'SC_C400LK_CR', 'SC_C400GH_CR', 'SC_CCMSBN_CR', 'SC_C400BH_CR') and
TRN_AUTH_POST = 'A' and
AUTH_DECISION_XCD = 'A' and
TRN_POS_ENT_CD IN ('E', 'K', 'G') and
TRN_TYP in ('C', 'M', 'P') and
UPPER(MER_NM) like '%GOOGLE%' and
USD > 0"))
...
##################### CR_GBL_SRVC_UDV_GOOGLE_DAILY_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, CR_GBL_SRVC_UDV_GOOGLE_DAILY_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### CR_GBL_SRVC_UDV_GOOGLE_DAILY_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$CR_GBL_SRVC_UDV_GOOGLE_DAILY_AMT <- unlist(tapply(dt1$TRN_AMT, cumsum(c(0, diff(dt1$CR_GBL_SRVC_UDV_GOOGLE_DAILY_CNT) < 1)), cumsum))
...
Country_Txn_Data_1$CR_GBL_SRVC_UDV_GOOGLE_DAILY_CNT <- as.numeric(as.character(Country_Txn_Data_1$CR_GBL_SRVC_UDV_GOOGLE_DAILY_CNT))
Country_Txn_Data_1$CR_GBL_SRVC_UDV_GOOGLE_DAILY_AMT <- as.numeric(as.character(Country_Txn_Data_1$CR_GBL_SRVC_UDV_GOOGLE_DAILY_AMT))
