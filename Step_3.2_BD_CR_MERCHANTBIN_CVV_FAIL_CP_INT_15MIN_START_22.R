...
setDT(dt1)[, CVV_FAIL_15MIN_CNT:=seq_len(.N), by=list(MERCHANTBIN, cumsum(dt1$group_10 >= 1))]

##################### CVV_FAIL_15MIN_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$CVV_FAIL_15MIN_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$CVV_FAIL_15MIN_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$CVV_FAIL_15MIN_CNT <- as.numeric(as.character(Country_Txn_Data_2$CVV_FAIL_15MIN_CNT))
Country_Txn_Data_2$CVV_FAIL_15MIN_AMT <- as.numeric(as.character(Country_Txn_Data_2$CVV_FAIL_15MIN_AMT))
