...
# Run this step in case we run Step 4 before Step 3
...
'SC_EURONETIN_DB', 'SC_HOGANHK_DB', 'SC_EURONETMY_DB', 'SC_EURONETSG_DB')"))
...
##################### DR_GBL_SRVC_UDV_CNP_3DS_30Mins_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DR_GBL_SRVC_UDV_CNP_3DS_30Mins_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### DR_GBL_SRVC_UDV_CNP_3DS_30Mins_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$DR_GBL_SRVC_UDV_CNP_3DS_30Mins_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$DR_GBL_SRVC_UDV_CNP_3DS_30Mins_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$DR_GBL_SRVC_UDV_CNP_3DS_30Mins_CNT <- as.numeric(as.character(Country_Txn_Data_2$DR_GBL_SRVC_UDV_CNP_3DS_30Mins_CNT))
Country_Txn_Data_2$DR_GBL_SRVC_UDV_CNP_3DS_30Mins_AMT <- as.numeric(as.character(Country_Txn_Data_2$DR_GBL_SRVC_UDV_CNP_3DS_30Mins_AMT))
