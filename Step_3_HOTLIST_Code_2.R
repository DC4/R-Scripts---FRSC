...
CNTY_BLACKLIST_MIDS_COUNTRYWISE_DEBIT <- BLACKLIST_MIDS_COUNTRYWISE_DEBIT
CNTY_BLACKLIST_MIDS_COUNTRYWISE_DEBIT <- NULL
CNTY_BLACKLIST_MIDS_BINATTACK_DEBIT <- BLACKLIST_MIDS_BINATTACK_DEBIT
CNTY_BLACKLIST_MIDS_BINATTACK_DEBIT <- NULL
Country_Txn_Data_3 <- rbind(BLACKLIST_MIDS_COUNTRYWISE_DEBIT,BLACKLIST_MIDS_BINATTACK_DEBIT)
Country_Txn_Data_3 <- NULL

Country_list <- list('AE', 'BH', 'BN', 'BW', 'CI', 'CM', 'GH', 'GM', 'IN', 'JO', 'KE', 'LK', 'NG', 'NP', 'QA', 'SG', 'SL', 'TZ', 'UG', 'VN', 'ZM', 'ZW')
...
for (i in 1:length(Country_list)) {
		...
		Country_Txn_Data_3 <- rbind(Country_Txn_Data_3, CNTY_BLACKLIST_MIDS_COUNTRYWISE_DEBIT, CNTY_BLACKLIST_MIDS_BINATTACK_DEBIT)
