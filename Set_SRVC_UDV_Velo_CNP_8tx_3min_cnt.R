...
SIC_CD <> ('5964' or '4900' or '5960' or '6300')"
)
...
dt[, Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_TIME := floor_date(as.POSIXct(date_2) + time_2, "30 minutes")]

# count and sum by 10 Minutes
dt[,`:=`(Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_CNT = as.numeric(seq_len(.N)), Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_AMT = as.numeric(cumsum(USD), digits = 2)) , by = .(ACCT_NBR, Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_TIME)]
...
dt1$Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_TIME = as.POSIXct("01.01.1960 00:00:00", format = "%d.%m.%Y %H:%M:%S", tz = "GMT");
dt1$Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_CNT = ""
dt1$Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_AMT = ""
...
Country_Txn_Data$Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_CNT <- as.numeric(as.character(Country_Txn_Data$Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_CNT))
Country_Txn_Data$Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_AMT <- as.numeric(as.character(Country_Txn_Data$Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_AMT))
...
UDV_Calculation_Table_Ref_1 <- Country_Txn_Data %>%
  select(ACCT_NBR, AUTH_DTTM_2, TRN_AUTH_POST, AUTH_DECISION_XCD, TRN_POS_ENT_CD, TRN_TYP, USD, date_2, time_2, Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_TIME, Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_CNT, Set_SRVC_UDV_Velo_CNP_8tx_3min_cnt_AMT)
