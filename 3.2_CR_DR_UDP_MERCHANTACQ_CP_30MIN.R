...
##################### CR_DR_MERACQ_CP_30MIN_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, CR_DR_MERACQ_CP_30MIN_CNT:=seq_len(.N), by=list(MERCHANTBIN, cumsum(dt1$group_10 >= 1))]

##################### CR_DR_MERACQ_CP_30MIN_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$CR_DR_MERACQ_CP_30MIN_AMT <- unlist(tapply(dt1$USD, cumsum(c(0, diff(dt1$CR_DR_MERACQ_CP_30MIN_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$CR_DR_MERACQ_CP_30MIN_CNT <- as.numeric(as.character(Country_Txn_Data_2$CR_DR_MERACQ_CP_30MIN_CNT))
Country_Txn_Data_2$CR_DR_MERACQ_CP_30MIN_AMT <- as.numeric(as.character(Country_Txn_Data_2$CR_DR_MERACQ_CP_30MIN_AMT))
