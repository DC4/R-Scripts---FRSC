...
##################### DB_GBL_SRVC_UDV_CNP_DAILY_CNT ##########################

# Getting the CUM_COUNT count variable

setDT(dt1)[, DB_GBL_SRVC_UDV_CNP_DAILY_CNT:=seq_len(.N), by=list(ACCT_NBR, cumsum(dt1$group_10 >= 1))]

##################### DB_GBL_SRVC_UDV_CNP_DAILY_AMT ##########################

# Getting the CUM_AMOUNT count variable

dt1$DB_GBL_SRVC_UDV_CNP_DAILY_AMT <- unlist(tapply(dt1$TRN_AMT, cumsum(c(0, diff(dt1$DB_GBL_SRVC_UDV_CNP_DAILY_CNT) < 1)), cumsum))
...
Country_Txn_Data_2$DB_GBL_SRVC_UDV_CNP_DAILY_CNT <- as.numeric(as.character(Country_Txn_Data_2$DB_GBL_SRVC_UDV_CNP_DAILY_CNT))
Country_Txn_Data_2$DB_GBL_SRVC_UDV_CNP_DAILY_AMT <- as.numeric(as.character(Country_Txn_Data_2$DB_GBL_SRVC_UDV_CNP_DAILY_AMT))
